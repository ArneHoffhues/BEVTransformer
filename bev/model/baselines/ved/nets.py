...
from bev.model.baselines.ved.utils import get_upsampling_weight
...
class encoder_after_vgg(nn.Module):
    ...
    def __init__(self, intermediate_size):
        ...
        self.mu_dec = nn.Linear(intermediate_size, 512)
        self.logvar_dec = nn.Linear(intermediate_size, 512)
...
class decoder_conv(nn.Module):
    def __init__(self, num_class, if_deconv):
        ...
        self.conv7 = double_conv(256, 256)
        #self.up8 = upsample(if_deconv=if_deconv, channels = 64)
        #self.conv8 = double_conv(64, 32)
...
class ved_model(nn.Module):

    def __init__(self, intermediate_size = 4096, num_class = 10):
        super(ved_model, self).__init__()

        self.vgg16 = models.vgg16_bn(pretrained=True)
        self.vgg16_feature = nn.Sequential(*list(self.vgg16.features.children())[:])
        self.encoder_afterv_vgg = encoder_after_vgg(intermediate_size = intermediate_size)
        self.decoder = decoder_conv(num_class = num_class, if_deconv=True)
