...
from bev.model.sfsegnet.mynn import Upsample, Norm2d
...
class AlignedModule(nn.Module):
    ...
    def flow_warp(self, input, flow, size):
        ...
        norm = torch.tensor([[[[out_w, out_h]]]], device=input.device).type_as(input)
        #h = torch.linspace(-1.0, 1.0, out_h).view(-1, 1).repeat(1, out_w)
        #w = torch.linspace(-1.0, 1.0, out_w).repeat(out_h, 1)
        h = torch.linspace(-1.0, 1.0, out_h, device=input.device).view(-1, 1).repeat(1, out_w)
        w = torch.linspace(-1.0, 1.0, out_w, device=input.device).repeat(out_h, 1)
