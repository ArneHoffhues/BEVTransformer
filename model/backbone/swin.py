...
def build_swin_fpn(variant: str, pretrained: bool, ckpt_type: Optional[str] = None):
    ...
    if pretrained:
        if variant == "base" and ckpt_type is None:
            ckpt_type = "cascade_mask_rcnn"
        elif variant in ("tiny", "small") and ckpt_type is None:
            ckpt_type = "mask_rcnn"

        ckpt_path = osp.join(get_ckpt_base_path(), f"{ckpt_type}_swin_{variant}_patch4_window7.pth")
...
def _test():
    model = build_swin_fpn(variant="tiny", pretrained=False).cuda()
    x = torch.ones(1, 3, 256, 1024).cuda()
