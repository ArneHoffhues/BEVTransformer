...
class RayProjectionLayer(nn.Module):
    ...
    def __init__(self, scales = [4, 8, 16, 32, 64], in_ch= 256, out_ch = 128, bev_grid_size=(49, 50), 
        ...
        self.volume_grid = construct_volume_lattice(bev_grid_size, grid_cell_width, vertical_sampling_range,
                n_samples)
    ...
    def forward(self, features, cam):
        ...
        cams = [torch.stack([cam[0] / scale, cam[1] /scale, cam[2]], dim=0) for scale in self.scales]
        projections = []
        for f, c in zip(features, cams):
            B, C, H, W = f.shape
            proj_points = torch.einsum('ij, klmj ->klmi', cam, self.volume_grid)
...
class Net3D(nn.Module):
    ...
    def __init__(self, scales = [4, 8, 16, 32, 64], in_ch= 256, out_ch = 512, n_classes = 11, 
            bev_grid_size=(49, 50), grid_cell_width=1, vertical_sampling_range=[-3,3], n_samples=50):
        super().__init__()        
        int_ch = in_ch // 2
    ...
    def forward(self, f, cam):
        ...
        out, collapsed = self.context_layer(x)
        return out, collapsed
